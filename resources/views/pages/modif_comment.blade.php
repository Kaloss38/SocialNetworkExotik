          ...
          <div class="col-xs-1 col-sm-1 col-md-1 col-lg-1"></div> <!-- Div morte (Div note) -->
  					<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 publication"> <!-- Colonne publication -->
  						...
  						<h2>Modifier votre commentaire</h2>
  							<form class="form-horizontal form-request" method="post" action="/home/2/{{$comments->id}}">
                  <input type="hidden" name="_method" value="put" />
  								{{ csrf_field() }}
                  <div class="form-group">
                    <input type="text" name="modif_comment" class="ameliorer" value="{{$comments->content}}">
  								</div>
  							  <div class="request-button">
                    <input type="submit" value="Envoyer" class="btn btn-primary">
                </div>
                </form>
                {{-- <form method="post" action="/home/{{$comment->id}}">
                {{ csrf_field() }}
